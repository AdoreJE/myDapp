...
let studentJSON = require("./solidity/build/contracts/Student.json");
let ABI = studentJSON.abi;
let Student = new web3.eth.Contract(ABI, CA);

// show the all student
async function showList() {
  console.log("showList() call");
  const tbody1 = document.getElementById("tbody1");

  // event가 발생하면 테이블 추가
  Student.events.SetStudentEvent({ fomBlock: "latest" }, (err, result) => {
    if (!err) {
  ...
  // 기존의 데이터 가져오기
  const studentList = await Student.methods.getStudentList().call();
  for (let i = 0; i < studentList.length; i++) {
    const result = await Student.methods.getStudent(studentList[i]).call();
...
async function insertTable(tbody, arglist) {
  ...
  cell3.innerHTML = await arglist.age;
